...
?>
<div class="wrap">
    <h1>List Booking Calendar - Du lịch khám chữa bệnh nhật bản </h1>
    ...
    <div id="normal-sortables" class="meta-box-sortables">
                        ...
                        if (cpabc_appointment_is_administrator() || ($current_user->ID == $item->conwer)) {
                            ...
                            <tr>
                                ...
                                <td nowrap><input type="text"
                                                  <?php if (!cpabc_appointment_is_administrator()) echo ' readonly '; ?>name="calname_<?php echo $item->id; ?>"
                                                  id="calname_<?php echo $item->id; ?>"
                                                  value="<?php echo esc_attr($item->uname); ?>"/></td>

                                <?php if (cpabc_appointment_is_administrator()) { ?>
                                    <td nowrap>
                                        <select name="calowner_<?php echo $item->id; ?>"
                                                id="calowner_<?php echo $item->id; ?>">
                                            <option value="0"<?php if (!$item->conwer) echo ' selected'; ?>></option>
                                            <?php foreach ($users as $user) {
                                                ?>
                                                <option
                                                    value="<?php echo $user->ID; ?>"<?php if ($user->ID . "" == $item->conwer) echo ' selected'; ?>><?php echo $user->user_login; ?></option>
                                            <?php } ?>
                                        </select>
                                    </td>
                                <?php } else { ?>
                                ...
                                <td nowrap><a
                                        href="<?php get_site_url(); ?>?cpabc_app=calfeed&id=<?php echo $item->id; ?>&verify=<?php echo substr(md5($item->id . $_SERVER["DOCUMENT_ROOT"]), 0, 10); ?>">iCal
                                        Feed</a></td>
                                <td nowrap>&nbsp; &nbsp;
                                    <?php if (cpabc_appointment_is_administrator()) { ?>
                                        <input style="font-size:11px;" type="button"
                                               name="calupdate_<?php echo $item->id; ?>" value="Update"
                                               onclick="cp_updateItem(<?php echo $item->id; ?>);"/> &nbsp;
                                    <?php } ?>
                                    <input style="font-size:11px;" type="button"
                                           name="calmanage_<?php echo $item->id; ?>" value="Manage Settings"
                                           onclick="cp_manageSettings(<?php echo $item->id; ?>);"/> &nbsp;
                                    <input style="font-size:11px;" type="button"
                                           name="calbookings_<?php echo $item->id; ?>" value="Bookings List"
                                           onclick="cp_BookingsList(<?php echo $item->id; ?>);"/> &nbsp;
                                </td>
                                <td nowrap style="font-size:10px;">[CPABC_APPOINTMENT_CALENDAR
                                    calendar="<?php echo $item->id; ?>"]
                                </td>
                            </tr>
                            <?php
                        }
        ...
        <?php if (cpabc_appointment_is_administrator()) { ?>
            ...
            <div id="metabox_basic_settings" class="postbox">
                ...
                <div class="inside">
                    <p><strong>Important!</strong>: Use this area <strong>only</strong> if you want to activate the form
                        builder or if you are experiencing conflicts with third party plugins, with the theme scripts or
                        with the character encoding.</p>
                    ...
                    <form name="updatesettings">
                        ...
                        <em>* The <strong>Visual Form Builder</strong> is available only in the <a
                                href="http://abc.dwbooster.com/download">pro version</a>. To edit the form in this basic
                            version you should manually edit the file 'cpabc_scheduler.inc.php'.</em>
                        ...
                        <select id="ccscriptload" name="ccscriptload">
                            <option
                                value="1" <?php if (get_option('CPABC_APPOINTMENTS_LOAD_SCRIPTS', (CPABC_APPOINTMENTS_DEFAULT_DEFER_SCRIPTS_LOADING ? "1" : "0")) == "1") echo 'selected'; ?>>
                                Classic (Recommended)
                            </option>
                            <option
                                value="2" <?php if (get_option('CPABC_APPOINTMENTS_LOAD_SCRIPTS', (CPABC_APPOINTMENTS_DEFAULT_DEFER_SCRIPTS_LOADING ? "1" : "0")) != "1") echo 'selected'; ?>>
                                Direct
                            </option>
                        </select><br/>
                        ...
                        <em>* Update the charset if you are getting problems displaying special/non-latin characters.
                            After updated you need to edit the special characters again.</em>

                        <br/><br/>
                        iCal timezone difference vs server time:<br/>
                        <select id="icaltimediff" name="icaltimediff">
                            <?php for ($i = -23; $i < 24; $i++) { ?>
                                <option value="<?php $text = " " . ($i < 0 ? "" : "+") . $i . " hours";
                                echo urlencode($text); ?>" <?php if (get_option('CPABC_CAL_TIME_ZONE_MODIFY_SET', " +2 hours") == $text) echo ' selected'; ?>><?php echo $text; ?></option>
                            <?php } ?>
                        </select><br/>
                        <em>* Update this, if needed, to match the desired timezone. The difference is calculated
                            referred to the server time.</em>

                        <br/><br/>
                        iCal timeslot size in minutes:<br/>
                        <input type="text" size="2" name="icaltimeslotsize" id="icaltimeslotsize"
                               value="<?php echo get_option('CPABC_CAL_TIME_SLOT_SIZE_SET', "10"); ?>"/> minutes
                        <br/>
    ...
    [<a href="http://abc.dwbooster.com/support?ref=callist" target="_blank">Documentation</a>]
    </form>
</div>