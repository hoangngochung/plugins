...
class wpdev_booking
{
    ...
    function __construct()
    {
        ...
        add_shortcode('booking_ed', array(&$this, 'booking_shortcode_ed'));
        add_shortcode('booking_form', array(&$this, 'booking_shortcode_form'));
...
//debuge($sql_req);
    ...
    function pre_get_calendar_html($bk_type = 1, $cal_count = 1, $bk_otions = array())
    {
        ...
        $calendar = $style .
            ...
            '<div style="display: none;" id="calendar_booking' . $bk_type . '">' .
        ...
        if ((!class_exists('wpdev_bk_personal')) && ($booking_is_show_powered_by_notice == 'On'))
            $calendar .= '<div style="display:none; font-size:9px;text-align:left;margin-top:3px;">Powered by <a style="font-size:9px;" href="http://wpbookingcalendar.com" target="_blank">WP Booking Calendar</a></div>';
        $calendar .= '<div class="col-md-12">';

        $calendar .= '<textarea class="col-md-12" id="date_booking' . $bk_type . '" name="date_booking' . $bk_type . '" autocomplete="off" style="display:none;"></textarea>';   // Calendar code
    ...
    function get_booking_form($my_boook_type)
    {
        ...
        $my_form .= '  <div class="col-md-6">[captcha]</div>';

        $my_form .= '  <button class="btn btn-default btn_bookiing" type="button" onclick="mybooking_submit(this.form,' . $my_boook_type . ',\'' . wpbc_get_booking_locale() . '\');" >' . __('Send', 'booking') . '</button> ';
    ...
    function get_booking_form_ed($my_boook_type)
    {
...
//debuge($bk_type);
    ...
    function add_booking_form_action_ed($bk_type = 1, $cal_count = 1, $is_echo = 1, $my_booking_form = 'standard', $my_selected_dates_without_calendar = '', $start_month_calendar = false, $bk_otions = array())
    {
        ...
        $my_result = ' ' . $this->get__client_side_booking_content_ed($bk_type, $my_booking_form, $my_selected_dates_without_calendar, $cal_count, $bk_otions) . ' ' . $start_script_code;
    ...
    // </editor-fold>
    function get__client_side_booking_content_ed($my_boook_type = 1, $my_booking_form = 'standard', $my_selected_dates_without_calendar = '', $cal_count = 1, $bk_otions = array())
    {
...
//debuge($attr);
    ...
    // Replace MARK at post with content at client side   -----    [booking_ed nummonths='1' type='1']
    function booking_shortcode_ed($attr)
    {
        //debuge($attr);
        ...
        $res = $this->add_booking_form_action_ed($my_boook_type, $my_boook_count, 0, $my_booking_form, '', $start_month_calendar, $bk_otions);

        return $res;
    }

    // FORM BOOKING
    function  booking_shortcode_form()
    {
        $uel = plugins_url().'/booking/js/datetimepicker.js';
        ...
        echo '
            ...
            <script src="'. $uel .'" type="text/javascript"></script>
            ...
            <link rel="stylesheet" type="text/css" href="http://maxcdn.bootstrapcdn.com/bootstrap/3.2.0/css/bootstrap-theme.min.css" media="screen" />
            <link rel="stylesheet" type="text/css" href="http://cdnjs.cloudflare.com/ajax/libs/jquery.bootstrapvalidator/0.5.0/css/bootstrapValidator.min.css" media="screen" />
            ...
            <!-- Form Name -->
            <legend>Du lịch khám bệnh nhật bản From</legend>

            <!-- Text input-->

            <div class="form-group">
              <label class="col-md-4 control-label hovaten">Họ và Tên</label>
            ...
            <div class="form-group">
              <label class="col-md-4 control-label hovaten">Furigana</label>
              <div class="col-md-4 inputGroupContainer">
              <div class="input-group">
                <span class="input-group-addon"><i class="glyphicon glyphicon-user"></i></span>
                <input  name="furigana_first_name" placeholder="First Name" class="form-control"  type="text">
                <input name="furigana_last_name" placeholder="Last Name" class="form-control"  type="text">
                </div>
              </div>
            </div>
...
<div class="form-group">
  ...
  <div class="col-md-4">
    <button type="submit1" class="btn btn-warning" >Send <span class="glyphicon glyphicon-send"></span></button>
  </div>
</div>
        ...
        global $wpdb;
        $id_booking = $wpdb->query($wpdb->prepare("SELECT booking_id FROM hc_booking"));
        $max_id_booking = $id_booking[0];
        var_dump($id_booking) ;

        if(isset($_REQUEST['submit1']))
        {
            $first_name = $_GET['first_name'];
            $wpdb->insert('hc_booking',array('booking_id'=>$max_id_booking+1,'form'=>$first_name,'booking_type'=>'1'),array('%d','%s','%d'));
    ...
    function bookingform_shortcode($attr)
    {
        ...
        }  //$my_selected_dates_without_calendar = '20.08.2010, 29.08.2010';
